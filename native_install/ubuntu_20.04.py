#!/bin/bash
#########################################################################################################
# Script to install Greenbone's openvas (GVM,GSM,GMP) on Ubuntu 20.04 server                            #
#                                                                                                       #
# Note: Running install scripts off the internet does have risks, please                                #
#       read through to understand what is hapening and update where needed                             #
#                                                                                                       #
#                                                                                                       #
# Usage: sudo ./ubuntu_20.04.sh                                                                         #
#                                                                                                       #
# Based on:                                                                                             #
# https://kifarunix.com/install-and-setup-gvm-11-on-ubuntu-20-04/                                       #
# https://www.cloudcybersafe.com/greenbone-vulnerability-manager-11-installation-on-ubuntu-from-source/ #
#                                                                                                       #
# Works 8-31-2020 on ubuntu 20.04 server iso and cloud image                                            #
#                                                                                                       #
# Prerequisites: 4GB Ram, 4vCPUs, 8GB Disk space                                                        #
#                                                                                                       #
#########################################################################################################
...
# Create GVM (non privileged) user to run GVM
...
python3-pip xsltproc nodejs graphviz texlive-fonts-recommended texlive-latex-extra --no-install-recommends
...
# GVM 11 has dropped support for sqlite3, so, postgreSQL must be installed
...
# Add GVM library path to /etc/ld.so.conf.d/gvm.conf
...
# If you encounter the error, ...error: ‘pcap_lookupdev’ is deprecated: use 'pcap_findalldevs'... while compiling openvas, 
# edit the CMakeLists.txt file and replace the line as shown below.
...
# the host scan information is stored temporarily on Redis servier
# config file is /etc/redis/redis.conf
ldconfig
cp /tmp/gvm-source/openvas/config/redis-openvas.conf /etc/redis/
chown redis:redis /etc/redis/redis-openvas.conf
echo "db_address = /run/redis-openvas/redis.sock" > /opt/gvm/etc/openvas/openvas.conf

# Update ownership of the configuration
...
# Redis background save may fail under low memory condition. To avoid this,
# enable memory overcommit (man 5 proc)
echo 'vm.overcommit_memory = 1' >> /etc/sysctl.conf
sysctl -p

# Disable Linux kernel's support for THP to avoid creation of latencies and memory usage issues with Redis
...
# Reload systemd configurations and enable on boot
systemctl daemon-reload
systemctl enable --now disable_thp
systemctl start redis-server@openvas
systemctl enable redis-server@openvas

# A number of NVTs require root privilege to perform operations,
# enable passwordless sudo for gvm user and update secure path
...
sudo -Hiu gvm echo "More info can be found by searching greenbone-nvt-sync rsyn connection refused" | sudo -Hiu gvm tee -a /opt/gvm/.bashrc
...
sudo -Hiu gvm echo "echo More info can be found by searching greenbone-nvt-sync rsync connection refused" | sudo -Hiu gvm tee -a /opt/gvm/.bashrc
...
# This should be run after nvt sync, but once a day is fine
HOUR=$(shuf -i 0-23 -n 1)
MINUTE=$(shuf -i 0-59 -n 1)
...
# Check the status
# Omitting this from the script, but you should run this command manually, or check /opt/gvm/var/log/gvm
...
sudo -Hiu gvm echo -e "gvmd --create-user gvmadmin --password=gvmadmin" | sudo -Hiu gvm tee -a /opt/gvm/.bashrc
...
echo "Username is gvmadmin and pasword is gvmadmin" 
