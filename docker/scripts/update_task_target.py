#!/usr/bin/env python3
#
# GMP script for gvm-pyshell to update OpenVAS task with new hostlist
...
#     --gmp-password <pass> \
#     <<ssh --hostname <host>/<tls>> /scripts/update_task_target.py host_file.csv \
#      <UUID-of-TASK>
...
def copy_send_target(gmp, hosts_file, old_target_id):
    ...
    keywords['name'] += "_" + now.strftime("%Y.%m.%d_%H.%M.%S")  # the name must differ from existing names
